...
<Tabs>
  <Tab title="Python">
    If you are calling `client.list_runs(is_root=True)` to enumerate traces, `client.traces.query()` is the dedicated method for that.
  ...
  <Tab title="TypeScript">
    If you are calling `client.listRuns({ isRoot: true })` to enumerate traces, `client.traces.query()` is the dedicated method for that.
...
<Tabs sync={false}>
  ...
  <Tab title="Java">
    If you are calling `client.runs().query()` with `isRoot(true)` to enumerate traces, `client.traces().query()` is the dedicated method for that.
...
<Tabs sync={false}>
  ...
  <Tab title="cURL">
    If you are calling `POST /api/v1/runs/query` with `is_root: true` to enumerate traces, `POST /v2/traces/query` is the dedicated method for that.

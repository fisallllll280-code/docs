...
Your configuration and data live separately under `~/.deepagents` — `config.toml`, `hooks.json`, a global `.env`, and a `.state/` directory holding sessions and saved credentials (see [App data](/oss/javascript/deepagents/code/data-locations)). These are **not** removed by the uninstall above. To also delete them:

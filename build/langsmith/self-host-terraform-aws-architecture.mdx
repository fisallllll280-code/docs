...
```txt
LangSmith Deployment add-on  (enable_deployments = true)
...
| cert-manager | `k8s-bootstrap` | `cert-manager` | Optional (Route 53 IRSA for DNS-01) | Automates TLS certificate issuance. Installed only when `tls_certificate_source = letsencrypt` or `create_cert_manager_irsa = true`. |

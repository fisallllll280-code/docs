...
The deployment runs in two stages: infrastructure (Terraform provisions VPC, GKE, Cloud SQL, Memorystore, GCS, Workload Identity) and application (the LangSmith chart, installed with the deploy script or the Terraform `app` layer). Add-ons are enabled with a flag and a redeploy.

```mermaid actions={false}
graph LR
    Start["setup-env.sh<br/>secrets to Secret Manager"]
    TF["Pass 1<br/>terraform apply"]
    Infra["VPC · GKE · Cloud SQL<br/>Memorystore · GCS · Workload Identity"]
    Bootstrap["Bootstrap workloads<br/>cert-manager · KEDA · Envoy Gateway"]
    Deploy["Pass 2 · make init-values + deploy<br/>helm install langsmith"]
    ...
    Start --> TF --> Infra --> Bootstrap --> Deploy --> DNS --> Cert --> Running
...
Three paths are supported. Pick one.

...
graph LR
    ...
    TF["Pass 1<br/>terraform apply (3 stages)"]
    Infra["AKS · PostgreSQL · Redis<br/>Blob · Key Vault · Managed Identity"]
    Bootstrap["Bootstrap workloads<br/>cert-manager · KEDA · ingress-nginx"]
    Secrets["make kubeconfig + k8s-secrets<br/>Key Vault to langsmith-config-secret"]
    ...
    Start --> TF --> Infra --> Bootstrap --> Secrets --> Deploy --> DNS --> Running

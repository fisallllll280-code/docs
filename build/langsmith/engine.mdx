...
Fixing an issue means applying the proposed code change and capturing the traces that surfaced it as eval examples, so the fix is both shipped and testable:

1. **Apply code change**
2. **Add eval examples**

You can mark the issue resolved directly from here once you're done — a shortcut for [resolving from Close](#close-resolve-ignore-or-reopen-an-issue) — or discard the fix to abandon it without resolving the issue. Discarding also stops watching the issue if it was being watched.

<Note>
Fixing is only available for open issues — [reopen](#close-resolve-ignore-or-reopen-an-issue) a resolved or ignored issue first.
</Note>

#### Open a pull request

Applying the fix means opening a GitHub pull request with the proposed code change in your connected repository — connect a repository first if you haven't. Once a pull request exists, LangSmith Engine links directly to it (with its branch), and reflects the PR's status — open, merged, or closed — throughout the issue. You can also copy the issue's fix context to your clipboard for use with an LLM or coding assistant. LangSmith Engine can propose code changes to any connected repository, including agents built with [Deep Agents](/oss/python/deepagents/overview), [LangChain](/oss/python/langchain/overview), and [LangGraph](/oss/python/langgraph/overview).
...
1. Open the traces behind the issue in the **Add as offline example** dialog. It shows each trace's input, the wrong output the agent produced, and a proposed expected output.
2. Add them directly to a dataset, or send them to an annotation queue to review first.
3. In the annotation queue, each example pairs the run's input with reference outputs proposed by LangSmith Engine, structured as named [assertions](/langsmith/assertions) — short claims about what a correct answer should or shouldn't include. Edit or add assertions as needed before saving each example to the dataset.
...
Watching keeps an issue open for monitoring without resolving or ignoring it — useful when you're not ready to fix it yet but still want to know if it keeps happening. LangSmith Engine confirms it's watching, and from there you can set up a Slack alert for the issue via **Alert me via Slack**, which jumps directly to the **Notifications** section of [Engine settings](#configure-langsmith-engine).

If new traces link to a watched issue, LangSmith Engine surfaces an alert with how many new traces arrived and moves the issue to the top of your list, so you can act on the recurrence — pick up the fix, or keep watching.

<Note>
Watching is only available for open issues without a pull request in flight — discard the fix to watch an issue again. Resolving or ignoring a watched issue automatically stops watching it.
</Note>
...
Closing records the outcome of your review: **Resolve** marks the issue as fixed, **Ignore** dismisses it as not real or not worth fixing. You can optionally provide a reason for either, which feeds back into LangSmith Engine's analysis.

A closed issue can be reopened at any time, which clears any fix in progress and stops watching the issue if it was being watched. LangSmith Engine also reopens an issue automatically if it detects the same problem recurring in a later trace.

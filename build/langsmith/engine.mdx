...
<CardGroup cols={2}>
  ...
  <Card title="Track: Watch an issue" icon="eye" href="#watch-keep-an-eye-on-an-issue">
    Keep an issue on your radar and get alerted when new traces link to it.
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="Connect a code repository (optional)">
    Although optional, connecting a GitHub repository is recommended. LangSmith Engine uses your source code to diagnose problems, generate higher-quality fixes, and open pull requests directly from issues. Under **Connect your agent's code repository**, select a repository. Only repositories the GitHub app can access are shown. Click **Manage app access →** to update permissions. You can update the **Code repository** at any time from [**Engine settings**](#configure-langsmith-engine).
  </Step>
...
The issue detail toolbar has three buttons — **Fix**, **Watch**, and **Close** — plus a priority dropdown. **Fix** and **Watch** each open a panel below the toolbar; **Close** opens a menu. Each button's own label and color reflect the issue's current state, so the toolbar itself shows you at a glance whether an issue has a PR in flight, is being watched, or is resolved.
...
Click **Fix** to open the fix panel, a three-step workflow:

1. **Apply code change**
2. **Add eval examples**
3. **Add a regression monitor**

At the bottom of the panel, click **Mark as Resolved** to resolve the issue directly — a shortcut for [resolving from Close](#close-resolve-ignore-or-reopen-an-issue) — or **Discard Fix** to close the panel without resolving. Discarding also stops watching the issue if it was being watched.

<Note>
**Fix** is disabled once the issue is resolved or ignored — [reopen it](#close-resolve-ignore-or-reopen-an-issue) first.
</Note>

#### Open a pull request

In step 1, connect GitHub (or a repository) if you haven't already, then click **Open PR** to open a GitHub pull request in your connected repository with the proposed fix applied. Once a pull request is open, this step shows the PR number — click it to open the pull request on GitHub — along with the branch name, and the toolbar's **Fix** button itself updates to show the PR number and its status (open, merged, or closed). Click **Copy Fix Context** to save a prompt with the issue details to your clipboard, for use with an LLM or coding assistant. LangSmith Engine can propose code changes to any connected repository, including agents built with [Deep Agents](/oss/python/deepagents/overview), [LangChain](/oss/python/langchain/overview), and [LangGraph](/oss/python/langgraph/overview).

#### Add offline examples

In step 2 (or from the **Linked Traces** section further down the page):

1. Click **Add offline examples** to open the **Add as offline example** dialog.
...
Once every example has been added, this step shows a confirmation instead of the button.
...
If LangSmith Engine has a suggested evaluator for the issue, step 3 shows it as a card:

1. Click the card to deploy the suggested evaluator.
...
Click **Watch** to open the watch panel and keep an issue on your radar without resolving or ignoring it. Click **Watch** again to stop watching.

The watch panel shows a confirmation banner with an **Alert me via Slack** button that jumps directly to the **Notifications** section of [Engine settings](#configure-langsmith-engine) to add a Slack destination.

If LangSmith Engine links new traces to a watched issue, the banner switches to an alert showing how many new traces arrived and moves the issue to the top of the list. From there you can pick up the fix workflow — connect GitHub, or open or view the pull request — or click **Keep Watching** to dismiss the alert and continue monitoring.

<Note>
**Watch** is disabled while the issue is resolved or ignored, and while it has an open pull request from Fix — discard the fix to watch it again. Resolving or ignoring a watched issue automatically stops watching it.
</Note>

Watching is tracked locally in your browser; it isn't shared with other users or synced across your devices.

### Close: resolve, ignore, or reopen an issue

Click **Close** to open a menu:

- Click **Resolve** to mark the issue as fixed, or **Ignore** to dismiss it as not real or not worth fixing. You can optionally provide a reason for either action.
- Once an issue is resolved or ignored, the **Close** button itself updates to show **Resolved** or **Ignored**, and the menu offers **Reopen** instead — click it to reopen the issue immediately, no reason needed. Reopening clears any fix in progress and stops watching the issue if it was being watched.

If the evaluator created from a resolved issue fires again on a future trace, the issue is automatically reopened to indicate the problem persists.
...
- **Priorities**: Areas LangSmith Engine should pay extra attention to when scanning traces. Changes take effect on the next scan.
...
- **Pause Engine**: LangSmith Engine scans your traces every 6 hours by default. Click **Pause** to suspend scanning or **Resume** to resume scanning.

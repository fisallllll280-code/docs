...
<CardGroup cols={2}>
  ...
  <Card title="Track: Watch an issue" icon="eye" href="#watch-an-issue">
...
Click **Open PR** to open a GitHub pull request in your connected repository with the proposed fix applied. Once a pull request is open, the first step of the fix workflow shows the PR number — click it to open the pull request on GitHub — along with the branch name, and the toolbar's **Fix** button updates to show the PR number and its status (open, merged, or closed). LangSmith Engine can propose code changes to any connected repository, including agents built with [Deep Agents](/oss/python/deepagents/overview), [LangChain](/oss/python/langchain/overview), and [LangGraph](/oss/python/langgraph/overview).

### Resolve or ignore an issue

Click **Resolve** to mark an issue as fixed, or **Ignore** to dismiss it as not real or not worth fixing. You can optionally provide a reason for either action. While reviewing a proposed fix, you can also click **Mark as Resolved** directly in the fix workflow as a shortcut, or **Discard Fix** to close the workflow without resolving the issue.

### Reopen an issue

To reopen a previously closed issue, open the issue detail view and click **Reopen**. Reopening starts the issue fresh: any code fix in progress and any active watch on the issue are cleared.

### Watch an issue

Click **Watch** to keep an eye on an issue without resolving or ignoring it. LangSmith Engine shows a confirmation banner in the issue detail view, and the list entry displays a watch indicator. Click **Watch** again to stop watching.

From the watch banner, click **Alert me via Slack** to jump directly to the **Notifications** section of [Engine settings](#configure-langsmith-engine) and add a Slack destination.

If LangSmith Engine links new traces to a watched issue, the banner switches to an alert showing how many new traces arrived and moves the issue to the top of the list. From there you can open a pull request (or connect GitHub or a repository, if you haven't yet) to act on the recurrence, or click **Keep Watching** to dismiss the alert and continue monitoring.

<Note>
Watch is disabled while an issue has an open pull request from Fix — discard the fix to watch the issue again. Resolving or ignoring a watched issue automatically stops watching it.
</Note>

...
graph LR
    ...
    Infra["VPC · EKS · RDS<br/>ElastiCache · S3 · ALB · IAM"]
    Bootstrap["k8s-bootstrap<br/>ESO · KEDA · cert-manager"]
    Deploy["Pass 2 · deploy.sh<br/>writes ARNs + hostname, ESO syncs secrets"]
    Helm["helm install<br/>langsmith chart"]
    First{"First<br/>deploy?"}
    Running["LangSmith running<br/>all pods healthy"]

    Start --> TF --> Infra
    Infra -->|EKS ready| Bootstrap --> Deploy --> Helm --> First
    First -->|no| Running
    First -->|yes, re-run with resolved hostname| Helm
...
Capacity has two independent levers:
...
Size the infrastructure for your target tier before deploying. See [Scaling guidance](/langsmith/self-host-scale) for per-tier recommendations.

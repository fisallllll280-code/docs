...
<Update label="2026-06-22" tags={["agent-server"]}>
## v0.12.0

- Updated pyjwt to version 2.13.0, incorporating multiple security fixes and compliance enhancements.
- Bumped dependencies: pyjwt to 2.13.0, ruff to 0.15.14, and pytest-asyncio to 1.4.0 for improved security and functionality.
- Updated the `hono` library from version 4.12.21 to 4.12.25, addressing several security vulnerabilities and other improvements.
- Updated the `hono` dependency from version 4.12.21 to 4.12.25 to include important security fixes.
- Updated `undici` from version 7.24.7 to 7.28.0, addressing multiple security vulnerabilities.
- Resolved issues with spurious errors during HITL interrupt responses by improving interrupt ID validation and ensuring fault tolerance in connection scenarios.
- Re-applied stranded indexes by renumbering migration 058 to 061 and introduced a migration HEAD sentinel to prevent future migration conflicts.
- Node.js in the Wolfi/Chainguard FIPS Agent Server now operates in FIPS mode with a validated OpenSSL provider for enhanced security compliance.
- Migrated agent server metrics to use the OpenTelemetry/Prometheus client, with renamed metric names for scrapers and updated latency measurements.
- Built the Agent Server Go core with FIPS 140-3 validated Cryptographic Module for improved security compliance.
</Update>
